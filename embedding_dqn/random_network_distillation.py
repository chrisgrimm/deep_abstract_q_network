...
class RND_Array(object):
    ...
    def update_state_normalizations(self, inp_s):
        assert inp_s.shape == tuple(self.inp_shape)
        self.last_n_frames.append(inp_s)
        self.last_n_frames = self.last_n_frames[-self.n:]
        self.s_mean = np.mean(self.last_n_frames, axis=0)
        self.s_std = np.std(self.last_n_frames, axis=0)

    def update_reward_normalizations(self, r):
        self.last_n_rewards.append(r)
        self.last_n_rewards = self.last_n_rewards[-self.n:]
        self.r_std = np.std(self.last_n_rewards)
    ...
    def get_intrinsic_rewards(self, inp_s, dqn_index):
        ...
        reward = self.sess.run([sq_diff], feed_dict={self.inp_s: inp_s,
                                                     self.inp_s_mean: self.s_mean,
                                                     self.inp_s_std: self.s_std})
        return reward / self.r_std
    ...
    def build_network(self, inp, name, reuse=None):
        with tf.variable_scope(name, reuse=reuse) as scope:
            ...
            N = tf.reduce_prod(tf.shape(c3)[1:], axis=0)
            c3 = tf.reshape(c3, [-1, N])
            fc1 = tf.layers.dense(c3, 512, tf.nn.relu, 'fc1')
