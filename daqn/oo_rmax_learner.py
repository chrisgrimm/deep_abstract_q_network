...
class OORMaxLearner(interfaces.LearningAgent):
    def __init__(self, config, environment, restore_file=None):
        ...
        self.transition_table = MovingAverageTable(100, self.pred_func)
        self.value_iteration = value_iteration.ValueIteration(self.gamma, self.max_VI_iterations, self.VI_delta)
        self.values = dict()
        self.qs = dict()
        ...
        self.l0_learner = DAQNLearner(self.config, self.environment)
        ...
        if restore_file is None:
            self.create_new_state(self.abs_func(self.environment.get_current_state()))
        else:
        ...
        while not self.environment.is_current_state_terminal() and (
            ...
            if self.using_global_epsilon:
                ...
                if type(a) is not L1ExploreAction and np.random.uniform(0, 1) < 0.1:
                    ...
                    eval_action = False
                    epsilon = max(self.min_epsilon, 1 - self.transition_table.get_success_rate(a))
                    sys.stdout.write('Executing action: %s -- eps: %.6f ... ' % (a, epsilon))
