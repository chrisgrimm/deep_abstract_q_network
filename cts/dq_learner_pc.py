...
class DQLearner(interfaces.LearningAgent):
    ...
    def __init__(self, dqn, num_actions, gamma=0.99, learning_rate=0.00025, replay_start_size=50000,
        ...
        self.cts = pc_cts.LocationDependentDensityModel((11, 11), lambda x, y: x, pc_cts.L_shaped_context)
        self.encoding_func = toy_mr_encoder.encode_toy_mr_state
        self.beta = 0.05

    def update_q_values(self):
        S1, A, R, S2, T, enc_S2, M1, M2 = self.replay_buffer.sample(self.batch_size)
    ...
    def run_learning_episode(self, environment, max_episode_steps=100000):
        ...
        for steps in range(max_episode_steps):
            ...
            self.replay_buffer.append(state[-1], action, R_plus, next_state[-1], enc_s, is_terminal)
