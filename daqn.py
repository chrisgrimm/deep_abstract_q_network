...
def hardmax(x, batch_size, I = None, sample=True):
...
def hook_abstraction(vis, num_abstract_states, batch_size):
    with tf.variable_scope('vis'):
        it_doesnt_matter = th.fully_connected(vis, num_abstract_states, lambda x: x)
    return hardmax(it_doesnt_matter, batch_size)
...
class L0_Learner:
    ...
    def __init__(self, sess, abstraction_scope, visual_scope, num_actions, num_abstract_actions, num_abstract_states,
        ...
        with tf.variable_scope(self.abstraction_scope, reuse=True):
            self.sigma_p = tf.stop_gradient(hook_abstraction(self.visual_output_sp, num_abstract_states, 1))
...
class L1_Learner:
    def __init__(self, num_abstract_states, num_actions, gamma=0.9, learning_rate=0.00025, replay_start_size=32,
                 epsilon_start=1.0, epsilon_end=0.1, epsilon_steps=100, replay_memory_size=100,
                 frame_history=4, batch_size=32, error_clip=1):
        ...
        with tf.variable_scope(self.abstraction_scope):
            self.sigma = hook_abstraction(self.visual_output, self.num_abstract_states, batch_size)
        ...
        with tf.variable_scope(self.visual_scope, reuse=True):
            ...
            self.visual_output_sp = hook_visual(masked_input_sp, self.frame_history)
        with tf.variable_scope(self.abstraction_scope, reuse=True):
            self.sigma_p = hook_abstraction(self.visual_output_sp, self.num_abstract_states, batch_size)
        ...
        self.saver = tf.train.Saver(var_list=th.get_vars('l1_online')+th.get_vars('online'))
    ...
    def update_q_values(self):
        # TODO: fix update and replay buffer to incorporate both l0 and l1 states
        S1, A, R, S2, T, M1, M2 = self.replay_buffer.sample(self.batch_size)
