...
from vae import network as vae_network


class DQLearner(interfaces.LearningAgent):

    def __init__(self, dqn, num_actions, max_vae_loss_buffer_size=10000, variance_max=3.0, gamma=0.99, learning_rate=0.00025, replay_start_size=1000,
                 epsilon_start=0.1, epsilon_end=0.1, epsilon_steps=1000000,
    ...
    def get_reward_bonus(self, S1):
        [_, z_variance, vae_loss] = self.sess.run([vae_network.train_op, vae_network.z_variance, vae_network.loss],
                                                  feed_dict={vae_network.inp_image: np.array(S1) / 255.})
        batch_max_loss, batch_min_loss = np.max(vae_loss), np.min(vae_loss)
        self.max_vae_loss_buffer.append(batch_max_loss)
        self.min_vae_loss_buffer.append(batch_min_loss)
        max_vae_loss = max(self.max_vae_loss_buffer)
        min_vae_loss = min(self.min_vae_loss_buffer)
        assert max_vae_loss > 0.0
        normalized_vae_loss = (vae_loss - min_vae_loss) / max_vae_loss
        bonus = (normalized_vae_loss)*self.variance_max + (1 - normalized_vae_loss)*np.clip(z_variance, 0., self.variance_max)
        return 0.5 * bonus / float(self.variance_max)
    ...
    def update_q_values(self):
        ...
        [_, loss, q_online, maxQ, q_target, r, y, error, delta, g] = self.sess.run(
            ...
            feed_dict={self.inp_frames: S1, self.inp_actions: Aonehot,
                       self.inp_sp_frames: S2, self.inp_reward: (R + RB)/1.5,
